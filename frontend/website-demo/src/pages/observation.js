...
function createData(id, sensor_name, observable_property, time_start, time_end, value) {
  return { id, sensor_name, observable_property, time_start, time_end, value }
}
...
export class ObservationTab extends React.Component {
  ...
  componentDidMount() {
    ObservationAPI.getAllObservations(false, (response) => {
      var observationsFromApi = response.message.map((observation) => {
        return new Observation(observation.id, observation.sensor_name, observation.observable_property,
      ...
      var resultRows = observationsFromApi.map((observation) => {
        ...
        return createData(observation.id, observation.sensor_name, observation.observable_property, 
